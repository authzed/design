...
- **No canonical combined-lockup asset (found 2026-09-11)** — `logo-brand.md` enumerates five canonical assets, none of which is a pre-combined logomark+wordmark lockup. The file everything actually reaches for, `projects/web/public/authzed-logo-multi.svg`, is an undocumented sixth variant AND carries the legacy drift hexes below. So the most common single action on any board ("place the AuthZed logo") ships colour the spec disowns, with no compliant asset to substitute. Fix is asset-side, not spec-side: produce a token-correct combined lockup and catalogue it.
